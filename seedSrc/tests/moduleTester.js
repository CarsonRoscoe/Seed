...
class ModuleTester{
    ...
    constructor(moduleName, startingUser, simulateDAG) {
        console.info("## Begin Testing Module " + moduleName);
    ...
    failTest(failMessage, actualResult) {
        this.totalCounter++;
        let msg = "Failed Test #" + this.totalCounter + ": " + failMessage;
     ...
     * If not, it will fail the test and log the failMessage
     * 
     * @param {*} expression - The expression to test for pass/fail
     * @param {*} failMessage - The fail message to log if the values do not match
     */
    assertExpression(expression, failMessage) {
        if (!expression) {
            this.failTest(failMessage);
            return false;
        } else {
     ...
     */
    endTest() {
        console.log("## " + this.moduleName + " Test Complete");

        let failed = this.fails.length > 0;
        let passed = this.fails.length == 0 && this.passCounter > 0;

        if (passed) {
            console.log("## Passed All " + this.totalCounter + " Asserts");
        } else if (failed) {
            console.log("## Scenario Test failed");
            console.log("## Passed " + this.passCounter + "/" + this.totalCounter + " Unit Tests");
            console.log("## Failed Tests..."); 
            console.log(this.fails);
        } else {
            console.info("## No Asserts Were Ran");
        }
    }
}